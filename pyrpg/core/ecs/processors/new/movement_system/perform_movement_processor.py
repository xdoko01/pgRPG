__all__ = ['PerformMovementProcessor']
...
class PerformMovementProcessor(Processor):
    ''' Updates Position component (position of an entity on the map) based on Movable
    component (movement).
    ...
    Related processors:
        -   PerformCommandProcessor - move commands generate movement
    ...
    def process(self, *args, **kwargs):
        ...
        for ent, (position, movable, flag_do_move) in self.world.get_components_ex(Position, Movable, FlagDoMove, exclude=FlagDoAttack):
            ...
            logger.debug(f'({self.cycle}) - Entity {ent} - new move vector {flag_do_move}')
            ...
            logger.debug(f'({self.cycle}) - Entity {ent} - new velocity {movable.velocity}, accelerate {movable.accelerate}')

            # Mark if the movment has stopped
            if movable.velocity == 0:
                self.world.add_component(ent, FlagHasStoppedMovement())
                logger.debug(f'({self.cycle}) - Entity {ent} - new flag "FlagHasStoppedMovement" assigned.')
            ...
            logger.debug(f'({self.cycle}) - Entity {ent} - new direction "{position.direction}".')
