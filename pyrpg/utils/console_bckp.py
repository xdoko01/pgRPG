...
class CommandLineProcessor(cmd.Cmd):
	...
	def __init__(self, app, lua_runtime=None, input=sys.stdin, output=sys.stdout, script_module_path=None):
		...
		# Dict with py scripts
		self._py_scripts = dict()


	def get_py_script(self, py_script_name):    
		'''Gets the py script module from the storage if registered or register it first.
		'''
		py_script_module = self._py_scripts.get(py_script_name)
		return py_script_module if py_script_module is not None else self.register_py_script(py_script_name)

	def register_py_script(self, py_script_module_name: str):
		'''Register the py script module.'''

		py_script_module_path_absolute = f"{self.script_module_path}.{py_script_module_name}"

		# Try to find the  module and get its reference
		try:
			py_script_module = str_to_package_module(None, py_script_module_path_absolute)

		except ValueError:
			print(f'Error during loading of py script module "{py_script_module_path_absolute}".')
			raise ValueError(f'Error during loading of py script module "{py_script_module_path_absolute}".')

		# Try to register the script
		try:
			py_script_module.initialize(self._register_py_script, py_script_module_name)
			print(f'Following py scripts are now registered. {self._py_scripts=}')

		except ValueError:
			print(f'Error during initiating/registering of command "{py_script_module_path_absolute}".')
			raise ValueError(f'Error during initiating/registering of command module "{py_script_module_path_absolute}".')

		return self._py_scripts.get(py_script_module_name)

	def _register_py_script(self, fnc, alias) -> None:
		'''Registers new script command under some
		specific name.
		'''
		self._py_scripts.update({alias: fnc})
	...
	def do_py_script(self, params):
		...
		'''
		# Try to get the py script name
		py_script = self.get_py_script(params.split()[0])
...
class Console(pygame.Surface):
	...
	def init(self, width: int, config: dict={}, app=None, lua_runtime=None):
		...
		# Initiace object for processing console commands - output of the class is redirected
		# if console_output is not defined then standard output is used (sustem text console)
		self.cli = CommandLineProcessor(self.app, self.lua, output=self.console_output, script_module_path=config.get('global').get('script_module_path', None)) if self.console_output else CommandLineProcessor(self.app)
