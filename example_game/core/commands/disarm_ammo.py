''' Module implementing DISARM_AMMO command 

For tests call python -m example_game.core.commands.disarm_ammo -v
...
def process(
    ...
    # Entity (dropper - probably player or NPC) must have HasWeapon component in order to successfully disarm the ammo
    has_weapon: HasWeapon = ecs_mng.try_component(entity_id, HasWeapon)

    # If Entity (figher) does not have HasWeapon component then it cannot disarm any ammo - FAILURE
    if has_weapon is None: return CommandStatus.FAILURE

    # Create FlagIsAboutToDisarmAmmo component to the entity (probably player or NPC)
    ecs_mng.add_component(entity_id, FlagIsAboutToDisarmAmmo(ammo=ammo_entity_id, type=ammo_type))
    logger.debug(f'{entity_id=} - component FlagIsAboutToDisarmAmmo created with params {ammo_entity_id=}, {ammo_type=}.')

    # Processor PerformDisarmAmmo now disarms the ammo
