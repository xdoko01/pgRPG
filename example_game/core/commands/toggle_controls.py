''' Module implementing TOGGLE_CONTROLS command 

For tests call python -m example_game.core.commands.toggle_controls -v
...
def process(
    ...
    # TODO - Switch between 2 different control profiles
    control_comp = ecs_mng.try_component(entity_id, Controllable)

    # Finish if there is no Controllable componet for the entity
    if not control_comp: 
        logger.debug(f'{entity_id=}. No Controllable component. Command skipped.')
        CommandStatus.FAILURE

    # Toggle based on if backup is done or not
    if control_comp.backup_control_cmds is None:
        control_comp.set_control_cmds(control_cmds=control_cmds)
        logger.debug(f'{entity_id=}. Input controls changed to {control_comp.control_cmds=}.')

        control_comp.set_key_feedback(key_feedback=key_feedback)
        logger.debug(f'{entity_id=}. Input key feedback mode changed to {control_comp.key_feedback=}.')

    else:
        control_comp.restore_control_cmds()
        logger.debug(f'{entity_id=}. Input controls restored to {control_comp.control_cmds=}.')

        control_comp.restore_key_feedback()
        logger.debug(f'{entity_id=}. Input key feedback mode  restored to {control_comp.key_feedback=}.')
