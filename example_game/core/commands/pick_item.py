''' Module implementing PICK_ITEM command 

For tests call python -m example_game.core.commands.pick_item -v
...
def process(
    ...
    # Entity (picker - probably player or NPC) must have HasInventory component
    has_inventory = ecs_mng.try_component(entity_id, HasInventory)

    # If Entity (picker) does not have HasWeapon component  - FAILURE
    if has_inventory is None: return CommandStatus.FAILURE

    # Create FlagIsAboutToPickEntity component to the entity (probably player or NPC)
    ecs_mng.add_component(entity_id, FlagIsAboutToPickEntity(entity_for_pickup=item_entity_id, category=item_category))
    logger.debug(f'{entity_id=} - component FlagIsAboutToPickEntity created with params {item_entity_id=}, {item_category=}')

    # Processor PerformPickProcessor now picks the item
