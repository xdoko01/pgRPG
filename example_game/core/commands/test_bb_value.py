''' Module implementing TEST_BB_VALUE command 

For tests call python -m pyrpg.core.commands.commands.new.test_bb_value -v
...
def init(
    ...
    Parameters:
        ...
        :param json_expr: json logic expression
        :type json_expr: list
...
def process(
    ...
    # Evaluate the expression on the ctx.globals
    res = json_logic(expr=json_expr, value_fnc=lambda x: x, script_fnc=lambda *args: None, data=ctx.globals.__dict__)
    logger.debug(f'{entity_id=}. Globals are {ctx.globals=}.')
    logger.debug(f'{entity_id=}. Expression {json_expr} evaluated as {res=}.')

    if res: return  CommandStatus.SUCCESS
