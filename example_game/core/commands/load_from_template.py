''' Module implementing LOAD_FROM_TEMPlATE command 

For tests call python -m example_game.core.commands.load_from_template -v
...
def process(
    ...
    # Get the ECS function from loading templates
    try:
        ecs_mng.load_stored_template(template_id=template_id, entity_id=entity_id)
        logger.debug(f'{entity_id=}. Components from {template_id=} loaded.')

    except ValueError:
        logger.debug(f'{entity_id=}. Failure in loading components from {template_id=}.')
