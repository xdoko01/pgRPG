...
def process(
    ...
    # In case that item has Weapon component, arm the weapon
    weapon = ecs_mng.try_component(inv_item_entity_id, Weapon)
    if weapon is not None:
        # Assign weapon in HasWeapon component to the correct type
        return cmd_arm_weapon(
            ecs_mng=ecs_mng,
            entity_id=entity_id,
            ctx=ctx,
    ...
    # No arming was done - item is not a weapon
    return CommandStatus.FAILURE
