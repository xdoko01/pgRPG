''' Module implementing USE_WEAPON command 

For tests call python -m example_game.core.commands.use_weapon -v
...
def process(
    ...
    # Entity (figher - probably player or NPC) must have HasWeapon component in order to successfully use the weapon
    has_weapon: HasWeapon = ecs_mng.try_component(entity_id, HasWeapon)

    # If Entity (figher) does not have HasWeapon component then it cannot use any weapon - FAILURE
    if has_weapon is None: return CommandStatus.FAILURE

    # Weapon type must exist otherwise it cannot be used - FAILURE
    if weapon_type not in Weapon.WEAPONS.keys(): return CommandStatus.FAILURE

    # Create WeaponInUse component to the entity (probably player or NPC)
    weapon_in_use: WeaponInUse = ecs_mng.try_component(entity_id, WeaponInUse)

    ecs_mng.add_component(entity_id, FlagSetWeaponIntoUse(type=weapon_type, prev_ent_ids=set(filter(lambda x: x is not None, has_weapon.weapons[weapon_in_use.type].values()))))
    logger.debug(f'{entity_id=} - component FlagSetWeaponIntoUse created with params {weapon_type=}.')
