''' Module implementing TEST_CAN_SEE command 

For tests call python -m pyrpg.core.commands.commands.new.test_can_see -v
...
def init(
    ...
    Parameters:
        ...
        :param entities: list of entities that we want to test for sight
        :type entities: list

        :param out_bb_key: blackboard key to which to write the entity spotted (optional)
        :type out_bb_key: str
    '''

    logger.debug(f'{entity_id=}. Starting init')

    can_see_comp = ecs_mng.try_component(entity_id, CanSee)
    ctx.locals.add('_can_see_comp', can_see_comp)
...
def process(
    ...
    if not ctx.locals._can_see_comp: 
        logger.debug(f'{entity_id=}. Entity does not have CanSee componment. Ending with FAILURE.')
        return CommandStatus.FAILURE

    # Check if entity is seen
    for e in entities:
        if e in ctx.locals._can_see_comp.ent_in_sight:
            ctx.globals.add(out_bb_key, e)

            logger.debug(f'{entity_id=}. Entity {e=} seen. Ending with SUCCESS.')
