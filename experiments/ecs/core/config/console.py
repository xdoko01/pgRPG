import core.config.paths as paths # for font path used in console
import core.config.config as cfg  # for console configuration stored in CONSOLE dict
import utils # for Console class
import importlib # for passing module name to COnsole as a string from config file
from core.engine import window # for window get width
...
game_console = utils.Console(
            importlib.import_module(cfg.CONSOLE.get('global').get('cli_module', 'core.engine')),
            window.get_width(),
            cfg.CONSOLE
        )