'''
    ...
    ###
        - images
            - dict that stores all key, value frame_id, Surface
            - only used frame_ids are stored, not necessarily the whole tilemap

        - frames
            - dict that store animation details
                - action (TILED SW parameter)
                - direction (TILED SW parameter)
                    - tiles
                    - repeat (TILED SW parameter)
                    - action_frame (TILED SW parameter)
        - actions
            - set of actions supported by the model
            - all actions will have automatically all directions (INIT part will ensure that)
       
    New model - INIT
    ###
        - Action must be in the list of supported actions - otherwise raise error
        - Action can have either 0 or all directions - otherwise raise error
        - Direction must be in the list of valid directions
        - Tiled SW json file does not need to have all the actions filled
            - idle action must always be present in the model!
        - Tiled SW json file contains Action without any direction
            - create all directions in frames dictionary for that action, containing the same data
        - Tiled SW json file contains Action that has not all directions specified
            - error is raised that some direction is not implemented
...

      {
        ...
        'walk' : {
            'up' : {
                ...
                'repeat' : True, # TILED SW parameter - OPTIONAL - it is always True unles it is set to false in the json parameters - False means that it stops on the last frame
...
__all__ = ['load_model', 'get_cache_info', 'clear_cache', 'Model2']
...
@functools.lru_cache(maxsize=32)    #Cash the models
class Model2(object):
        ...
        for anim_tile in model_data.get('tiles', []):
            ...
            if action in Model2.ACTIONS:
                ...
                if not direction:
                    ...
                    for new_dir in Model2.DIRECTIONS:
                ...
                elif direction in Model2.DIRECTIONS:
    ...
    def _check_model(self):
        ...
        for action, dir_dict in self.frames.items():
            for direction in Model2.DIRECTIONS:
