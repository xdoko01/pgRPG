''' Experiments with ECS
    ...
    Solved situations
        ...
        A: All movement (even corrective movements after collision) must be implemented
           as move command and pushed into command queue. Otherwise, by correction using
           lastx, lasty etc. entity will get stuck eventually.
    ...
    ########
        -	Architecture Design implemented
            ...
            - Maps and quests are accessed from Processors that have engine function as a input
              parameter i.e. they know how to call outside the esper.world

              - event handling - is achieved by calling to engine functions from esper.world
                processors
              - command handling - is achieved by calling to engine functions from esper.world
                processors
        ...
        -	Teleports implemented
            ^^^^^^^^^^^^^^^^^^^^^
            - remember collision entities on Colision components
        ...
        -	Quest event (simple) processing implemented
            ...
            - The engine handler function is called and takes the list of events and passes
              it to all quests
            - Each quest has event processor that processes the event based on conditions
              and actions
            ...
            - Actions are specified as a list of functions
                ...
                - modify_brain function - resets brain of any entity with commands from
                  Command class
        ...
        -	Animated Characters implemented
            ...
            - new Status component
                - needed to represent different statuses for different animations - 'idle',
                  'walk', ...
        ...
        -	Picking up and Wearing clothes implemented
            ...
            - typically wearable object will also have RenderableModel, Pickable, Collidable
              components
        ...
        -	Picking up a weapon and attacking implemented
            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            - new component Weapon - identification of the weapon type - 4 types supported
              (sword, spear, bow, spell)
                - typically weapon will also have following components - Pickable, Collidable,
                  RenderableModel
            - new component HasWeapon - if entity can arm a weapon and use a weapon
            - new component Factory - component that can generate other entities based on dict
              parameter 'prescription'
                - necessary to produce projectiles from the weapon
            - new component Container - component that is keeping reference to some other component
                - necessary to keep reference on projectile to original HasWeapon component with
                  list of generated projectiles.
            - new processor RenderableModelAnimationUpdateProcessor
                - updates animation frame only once for every entity that is displayed. Previous
                  solution of RenderWorldProcessor was updating the frame of entity for every camera
                  where this entity was present.
            - new processor RenderableModelAnimationActionProcessor
                - updates the action of the entity - entity can be idle, walk, idle weapon
                  and weapon action.
            - adjustment of processor RenderableModelWorldProcessor
                - newly the processor is calling get_current_frame fnction (no frame shifts)
                  instead of get_frame function (frame shifts)
            - new command attack
                - only sets has_attacked flag to True. The flag is used to decide of the action
                  of the entity
            - further notes
                - last attack animation frame creates new entity (projectile) by calling
                  HasWeapon -> Factory
                - there is new collision processor CollisionDamageProcessor that decreases health
                  represented by Damagable component


	- TODO
        - create map from the quest definition, not from the code
            - quest calls engine.create_map
            
        - create multiple maps from the list

        - Phase as a separate class of Quest
            - load quest 
                -> load first phase
                    -> do cleanup
                    -> load maps
                    -> load entities
                -> change set phase command to do all the neccesary changes


        - create processors in the quest specification - DO NOT DO THIS NOW
            - doable but complicated, because we need to pass references to global variables in the
		...
		- model system for weapons in IO
		- fix tex_actions ()
