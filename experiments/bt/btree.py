...
from get_class_object import str_to_class
from get_dict_params import get_dict_params

from pathlib import Path
from functools import reduce, partialmethod

BTREE_PATH = Path('experiments/bt/')


Command = namedtuple('Command', ['name', 'entity', 'params'])
...
class BTree(CommandGenerator):
    ...
    def __init__(self, tree_def: dict, cmd_factory=(lambda x: x), template_path: Path=Path(''), val_check: bool=False) -> None:
        ...
        self._root_node = create_tree(tree_def=tree_def['tree'], cmd_factory=cmd_factory, template_path=template_path)
        ...
        self.is_completed = self._root_node.is_completed
...
if __name__ == '__main__':
    ...
    btree = BTree(tree_def=b_tree_data_with_templates, template_path=BTREE_PATH, val_check=True)
    btree.print_tree()
