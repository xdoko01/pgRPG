...
#
# These are factory functions rather than module-level dicts so every test gets
# its own definition. Sharing one dict across tests couples them together, and
# a definition is exactly the kind of thing a generator may legitimately keep a
# reference to.
...
def test_btree_get_command_returns_command():
    ...
    cmd, is_first_call = btree.get_command()
...
def test_btree_restart_brain():
    ...
    # The tree must be runnable again after a restart, which is only possible if
    # the original definition survived the first build intact.
    cmd, is_first_call = btree.get_command()
    assert cmd is not None
    assert is_first_call
