"""Manage scriptable dialog flow definitions.

Load, store, and remove dialog objects built from JSON template files.
Dialogs are rendered as pygame surfaces via ``pgrpg.utils.dialog``.

Module Globals:
    _dialogs: Dictionary mapping dialog id strings to dialog surface objects.
"""
...
import pgrpg.utils.dialog as dialog

from pgrpg.core.config import FILEPATHS # DIALOG_PATH, IMAGE_PATH, FONT_PATH, Path # for DIALOG_PATH, IMAGE_PATH, FONT_PATH
from pgrpg.functions.get_dict_from_json import get_dict_from_json
...
def load_dialog(dialog_def: dict) -> None:
    """Create a dialog from a dictionary definition and store it in _dialogs.

    Resolves template inheritance, merges the final definition, then
    generates pygame surface objects.

    Args:
        dialog_def: Scene-level dialog definition dict. Must contain an
            ``"id"`` key. May contain a ``"templates"`` list.
    ...
    def prepare_dlg_data_from_template(dlg_template: str) -> dict:
        """Return a merged dictionary from a dialog template JSON file.

        Args:
            dlg_template: Template name (without ``.json`` extension).

        Returns:
            Merged template dictionary with inherited templates applied.
